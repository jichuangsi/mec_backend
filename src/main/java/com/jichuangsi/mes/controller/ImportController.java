...
public class ImportController {
    ...
    @ApiImplicitParams({})
    @PostMapping("/importFiletProstandards")
    @OperLog(operModul = "导入",operType = "2",operDesc = "产品管理-规格型号-导入操作")
    public ResponseModel importFiletProstandards(@RequestParam("file")MultipartFile file) throws IOException {
        try {
            // 获取传入文件
